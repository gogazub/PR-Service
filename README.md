...
1. **Нагрузочное тестирование (k6)**  (Частично выполнено)
...
Результаты тестирования лежат в файле `stress-tests-results.txt`

### 1. `/health` — проверка HTTP-стека (без БД)

- **Пропускная способность:** ~17 000 RPS  
- **p99 latency:** ~16 ms  

---

### 2. `/users/getReview` — чтение с участием базы данных

- **Пропускная способность:** ~1 175 RPS  
- **Средняя задержка:** ~40 ms  
- **p95 latency:** ~100 ms  
- **Конкурентность:** 50 потоков  

---

### 3. `CreatePR` (тест через k6) — сложная транзакционная операция

- **Пропускная способность:** ~752 RPS  
- **Средняя задержка:** ~26 ms  
- **p95 latency:** ~45 ms  
- **Конкурентность:** 20 VUs  

Операция включает:  
- 4 SELECT (проверка PR, пользователя, команды, активных участников),  
- 2 INSERT,  
- транзакцию с Commit. 